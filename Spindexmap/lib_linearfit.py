...
def twopoint_spidx_bootstrap(freq, flux, flux_err, niter=10000):
    ...
    random_flux = np.resize(flux, (niter, nsource, 2)) + np.resize(flux_err, (niter, nsource, 2)) * np.random.randn(
        niter, nsource, 2)
...
def plotlinax(data, plotname):
    ...
    # take the log10
    thisdata = {'flux': np.log10(data['flux']), 'freq': np.log10(data['freq']),
                'rms': 0.434 * data['rms'] / data['flux']}
...
if __name__ == "__main__":
    ...
    data = np.loadtxt(datafile, comments='#',
                      dtype=np.dtype({'names': ['freq', 'flux', 'rms'], 'formats': [float, float, float]}))
