...
def correct_beam_header(header):
    ...
    """
    if ('BMAJ' in header) and ('BMIN' in header) and ('PA' in header):
        return header
...
class AllImages():
    ...
    def align_catalogue(self):
        ...
        for i, image in enumerate(self):
            ...
            logging.info(
                f'{image.imagefile} Applying shift: {3600 * np.average(dra, weights=flux):.5f}", {3600 * np.average(ddec, weights=flux):.5f}".')
            # debug output
    ...
    def common_beam(self, circbeam=True):
        ...
        if circbeam:
            ...
            target_beam = [common_beam.major.to_value('deg'), common_beam.minor.to_value('deg'),
    ...
    def regrid_common(self, size=None, region=None, pixscale=None, radec=None, square=False, action='regrid'):
        ...
        if region:
            ...
            size = [2.2 * np.max([np.max([np.abs(dec_max - mdec), np.abs(dec_min - mdec)]),
                                  np.max([np.abs(ra_max - mra), np.abs(ra_min - mra)])])]
...
class Image(object):
    ...
    def calc_noise(self, niter=1000, eps=None, sigma=5, bg_reg=None, force_recalc=False):
        ...
        if bg_reg is not None:
            ...
            for i in range(niter):
                ...
                logging.debug(
                    '%s: MAD noise: %f uJy on %f%% data' % (self.imagefile, mad * 1e6, 100 * len(data) / initial_len))
    ...
    def convolve(self, target_beam, stokes=True):
        ...
        beam = self.get_beam()
        try:
            if (np.abs((target_beam[0] / beam[0]) - 1) < 1e-2) and (np.abs((target_beam[1] / beam[1]) - 1) < 1e-2) and (
                    np.abs(target_beam[2] - beam[2]) < 1):
            ...
            pass  # catch case where we have delta scale beam (model image)
        ...
        if stokes:  # if not stokes image (e.g. spectral index, do not renormalize)
            self.img_data *= (target_beam[0] * target_beam[1]) / (
                        beam[0] * beam[1])  # since we are in Jy/b we need to renormalise
    ...
    def apply_shift(self, dra, ddec):
        ...
        logging.info(
            'Shift %.2f %.2f arcsec (%s)' % (dra * 3600 * np.cos(self.dec * np.pi / 180.), ddec * 3600, self.imagefile))
    ...
    def make_catalogue(self):
        ...
        if not os.path.exists(img_cat):
            ...
            bdsf_img.write_catalog(outfile=img_cat.replace('.cat', '.skymodel'), catalog_type='gaul', format='bbs',
                                   bbs_patches='source', clobber=True, srcroot='src')
        else:
        ...
        logging.debug('%s: Number of sources detected: %i; removed %i extended sources.' % (
