...
parser.add_argument('-s', '--size', default=400, type=float, help='Size of the final image in arcsec. Default to 400 arcsec')
...
parser.add_argument('--stretch', default='linear', help='Stretch to apply to image. Default: linear for radio, power for X.')
...
if plottype=='radio':
    ...
    if interval:
        f.show_colorscale(cmap='gnuplot2', stretch=imstretch, vmin=float(interval[0]), vmax=float(interval[1]))
    else:
        f.show_colorscale(cmap='gnuplot2', stretch=imstretch)
    f.add_colorbar()
    ...
    f.recenter(center[0], center[1], radius=(size/3600.))
    if interval:
        f.show_colorscale(cmap='gnuplot2', stretch=imstretch, vmin=float(interval[0]), vmax=float(interval[1]), smooth=imsmooth)
    else:
        f.show_colorscale(cmap='gnuplot2', stretch=imstretch, smooth=imsmooth)
    f.add_colorbar()
    f.colorbar.set_width(0.15)
