...
def get_freq(ms: str):
    ...
    print("%s: Freq range: %f MHz - %f MHz (bandwidth: %f MHz, mean freq: %f MHz)" % (ms, min_freq, max_freq, bandwidth,
                                                                                      mean_freq))
...
def get_flags(ms: str):
    with pt.table(ms + '/FLAG_CMD', ack=False) as t:
        for a in t:
            print(a)
...
def get_cols(ms: str):
    ...
    default_colnames = ['UVW', 'FLAG_CATEGORY', 'WEIGHT', 'SIGMA', 'ANTENNA1', 'ANTENNA2', 'ARRAY_ID', 'DATA_DESC_ID',
                        'EXPOSURE',
