...
def get_models():
    ...
    models.append(TheilSenRegressor(
        fit_intercept=False))  # Strunggling a bit with this one as the output varies a lot given n_samples (if n_samples=1 then it returns the median of the ratio, if it equals the number of data points then it returns essentially the output of least square fitting)
    return models
...
def plot_best_fit(X, y, xaxis, model):
    ...
    print(modelname, 'Gradient', gradient, 'intercept', intercept,
...
parser.add_argument('--fieldname', type=str, default='LoTSS-DR2',
...
if not os.path.exists(infile.replace('.fits', 'cat.srl.fits')):
    img = bdsm.process_image(infile, thresh_isl=4.0, thresh_pix=5.0, rms_box=(150, 15), rms_map=True, mean_map='zero',
                             ini_method='intensity', adaptive_rms_box=True, adaptive_thresh=150,
                             rms_box_bright=(60, 15), group_by_isl=False, group_tol=10.0, output_opts=True,
                             output_all=True, atrous_do=True, atrous_jmax=4, flagging_opts=True, flag_maxsize_fwhm=0.5,
                             advanced_opts=True, blank_limit=None, frequency=restfrq)
    img.write_catalog(outfile=infile.replace('.fits', 'cat.srl.fits'), catalog_type='srl', format='fits',
                      correct_proj='True')
    img.write_catalog(outfile=infile.replace('.fits', 'cat.srl.reg'), catalog_type='srl', format='ds9',
                      correct_proj='True')
# LoTSS-DR2 catalogue
# Filter DR2 cat
...
lotsssnr = lotssdr2['Peak_flux'] / (2.0 * lotssdr2['E_Peak_flux']) + lotssdr2['Total_flux'] / (
...
print('MODELNAME', 'FITTEDGRAD', 'FITTEDINTERCEPT', 'FIT_MEAN_ABS_ERROR', 'FIT_MEAN_ABS_ERROR_STD', 'NUM SOURCES FIT',
      'MED SOURCE RATIOS', 'MEAN SOURCE RATIOS', 'STD SOURCE RATIOS')
print('BEST', bestmodel, bestgradient, bestintercept, bestmae, bestmaestd, len(y), np.median(ratios), np.mean(ratios),
      np.std(ratios))
