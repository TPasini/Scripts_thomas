...
print('')
print('########################################')
print('#### X-RAY THERMODYNAMICAL PROFILES ####')
print('########################################')
print('')
...
print(' - min bin radius in arcsec;')
print(' - max bin radius in arcsec;')
...
print(' - Metallicity (Solar)')
print('')

datafile = np.loadtxt('norm.txt')
kt = np.loadtxt('kt.txt')

if os.path.exists('res_tc.txt'):
    os.remove('res_tc.txt')
...
with open('elec_density.txt', 'w') as f:
    ...
    #writer.writerow(['#rmin(kpc)', 'rmax(kpc)', 'Num_density(cm^-3)', 'lower_numdensity','upper_numdensity', 'Density(g/cm^3), kT(keV)', 'Z(solar)'])
    ...
    for i,reg in enumerate(datafile):
        ...
        writer.writerow(["%3.3f" % rmin.value, "%3.3f" % rmax.value, "%.5f" % ne.value,"%.5f" % absngiu, "%.5f" % absnsu,"%.3e" % rho.value, kt[i,0], kt[i,3]])
...
with open('Results.txt', 'w') as g:
    ...
    for i, density in enumerate(dens):
        ...
        #TCOOL ESTIMATE
        with open('fortcool.txt', 'w') as g:
            writer2 = csv.writer(g, delimiter=" ", quoting=csv.QUOTE_NONE, escapechar=' ')
            writer2.writerow([kt[i,0], kt[i,3], dens[i,2], redshift])
        f = open('res_tc.txt','w+')
        os.system('./tcool-X.exe')
        readres = np.loadtxt('res_tc.txt')
        t_cool = float(readres)
        #print(t_cool)
        f.close()
        ...
        if os.path.exists('fortcool.txt'):
            os.remove('fortcool.txt')
...
if os.path.exists('elec_density.txt'):
